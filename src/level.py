    ...
    with open(level) as file:
        ...
        while line:
            ...
            if data["type"] == "obj":
                ...
                spawner_list.append(obj)
            elif data["type"] == "trapdoor":
                obj = TrapDoor(tile_size * int(data["x"]), tile_size * int(data["y"]), tile_size * int(data["w"]),
                               tile_size * int(data["h"]))
                obj.imgs["open"] = data["open"]
                obj.imgs["closed"] = data["closed"]
                # finalizes state
                obj.close()
                obj.spawner = Spawner()
                obj.spawner.load(data["file"], tile_size)
                env_obj_list.append(obj)
                spawner_list.append(obj)
